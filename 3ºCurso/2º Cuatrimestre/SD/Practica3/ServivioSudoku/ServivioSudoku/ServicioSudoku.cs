...
{
    public class ServicioSudoku:MarshalByRefObject, GestorJuegos
    {
        ...
        public float Completo(int Codigo)
        {
            float total = 0; ;
            Quesudoku(Codigo);
            if (actual.Dificultad != TipoDificultad.VACIO)
            {
                for (int i = 0; i < 9; i++)
                {
                    for (int j = 0; j < 9; j++)
                    {
                        char valor = ObtenerValorAux2(i, j);
                        if (valor != ' ')
                        {
                            total += Convert.ToInt32(valor)-48;
                        }
                    }
                }
                total = (total / 81) * 20;
        ...
        char ObtenerValorAux2(int pFila, int pColumna)
        {
            return actual.Tablero[pFila, pColumna];
        }
